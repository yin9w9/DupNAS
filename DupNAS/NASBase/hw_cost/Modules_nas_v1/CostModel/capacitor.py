...
# Vpce: The voltage where recharging starts from (proactive shutdown is initiated)
...
def cal_cap_recharge_time_custom(Epc, C=0.0001, R=1000, Vsup=3, Von=2.8, Voff=2.4, safe_margin=1.0):    
    Eavail = 0.5 * C * ((Von**2) - (Voff**2))   # available capacitor energy
    
    # check first
    if (Epc <= 0) or (Eavail * safe_margin <= Epc) or (Eavail < 0) or (Vsup < Von) or (Voff > Von):
