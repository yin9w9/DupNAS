...
def est_cost_CONV_contpow_notiled(layer, plat_settings, plat_cost_profile):  
    #E_rb, L_rb = est_cost_CONV_reboot_contpow(plat_cost_profile) # not included in model
    L_fd = est_cost_CONV_layerinputfetch_contpow_notiled(layer, plat_cost_profile)    
    L_cp = est_cost_CONV_layercomp_contpow_notiled(layer, plat_cost_profile)
    ...
    total_latency = L_fd + L_cp
    #total_energy
    return total_latency, cost_breakdown


# get layer data input fetch energy cost (overall layer)
def est_cost_CONV_layerinputfetch_contpow_notiled(layer, plat_cost_profile):
    total_lat_cost = 0  
    ...
    nI, nW, nO, blkI, blkW, blkO = _num_datatrcmds_fetch_notile_data(layer, params_exec)
    ...
    lr_blkI = plat_cost_profile['L_DMA_NVM_TO_VM'](blkI)
    lr_blkW = plat_cost_profile['L_DMA_NVM_TO_VM'](blkW)
    lr_blkO = plat_cost_profile['L_DMA_NVM_TO_VM'](blkO)
    
    # energy/latency overhead of each transfer
    lofI = plat_cost_profile['L_FD_I_OVHD']
    plat_cost_profile['L_FD_W_OVHD']
    ...
    if layer['optype'] in (OPTYPES.O_CONV1D_DW, OPTYPES.O_CONV2D_DW):
        ...
        if inter_lo == "reuse_I":
            ...
            total_lat_cost = ((nI*(lr_blkI + lofI))) + ((nW * (lr_blkW + lofW))) + (0*(nO * (lr_blkO + lofO_ruI)))

        elif inter_lo == "reuse_W":
            #total_en_cost = ((num_tiles)*(nI*(er_blkI + eofI))) + ((Mtm)*(nW * (er_blkW + eofW))) + (0*(nO * (er_blkO + eofO_ruW)))
            total_lat_cost = ((nI*(lr_blkI + lofI))) + ((nW * (lr_blkW + lofW))) + (0*(nO * (lr_blkO + lofO_ruW)))
        ...
        # partial sums are always in VM, so no need to refetch
            ...
            total_lat_cost = ((nI*(lr_blkI + lofI))) + ((nW * (lr_blkW + lofW)))
        ...
        if inter_lo == "reuse_I":
            ...
            total_lat_cost = ((nI*(lr_blkI + lofI))) + ((nW * (lr_blkW + lofW))) + ((nO * (lr_blkO + lofO_ruI)))

        elif inter_lo == "reuse_W":
            #total_en_cost = ((num_tiles)*(nI*(er_blkI + eofI))) + ((Mtm*Ntn)*(nW * (er_blkW + eofW))) + (ngt0*(nO * (er_blkO + eofO_ruW)))
            total_lat_cost = ((nI*(lr_blkI + lofI))) + ((nW * (lr_blkW + lofW))) + ((nO * (lr_blkO + lofO_ruW)))
